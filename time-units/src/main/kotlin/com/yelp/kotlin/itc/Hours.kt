...
class Hours internal constructor(nanos: Long) : Duration(nanos) {
    override operator fun plus(other: Duration) = Hours(nanos + other.nanos)
    override operator fun minus(other: Duration) = Hours(nanos - other.nanos)
}

val Duration.asHours: Hours get() = Hours(nanos)
